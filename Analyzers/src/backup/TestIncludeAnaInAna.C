#include "TestIncludeAnaInAna.h"

TestIncludeAnaInAna::TestIncludeAnaInAna(){

}

void TestIncludeAnaInAna::initializeAnalyzer(){
  ...
  cout << "[TestIncludeAnaInAna::initializeAnalyzer] RunSyst = " << RunSyst << endl;

  //==== Dimuon Z-peak with two muon IDs
  //==== I defined "vector<TString> MuonIDs;" in Analyzers/include/TestIncludeAnaInAna.h
  MuonIDs = {
    //"POGMedium",
    "POGTight"
  };
  ...
  cout << "[TestIncludeAnaInAna::initializeAnalyzer Setting ProcessName = " << ProcessName << endl;

  //==== (Example) Year-dependent variables
  //==== I defined "TString IsoMuTriggerName;" and "double TriggerSafePtCut;" in Analyzers/include/TestIncludeAnaInAna.h 
  ...
  cout << "[TestIncludeAnaInAna::initializeAnalyzer] IsoMuTriggerName = " << IsoMuTriggerName << endl;
  cout << "[TestIncludeAnaInAna::initializeAnalyzer TriggerSafePtCut = " << TriggerSafePtCut << endl;
  ...
  cout << "[TestIncludeAnaInAna::initializeAnalyzer] RunNewPDF = " << RunNewPDF << endl;
  if(RunNewPDF && !IsDATA){
  ...
  cout << "[TestIncludeAnaInAna::initializeAnalyzer] RunXSecSyst = " << RunXSecSyst << endl;
...
void TestIncludeAnaInAna::executeEvent(){
  ...
  //==== So, I defined "vector<Muon> AllMuons;" in Analyzers/include/TestIncludeAnaInAna.h,
  ...
  //==== I defined "double weight_Prefire;" in Analyzers/include/TestIncludeAnaInAna.h
  ...
  if(RunNewPDF && !IsDATA){
    //cout << "[TestIncludeAnaInAna::executeEvent] PDF reweight = " << GetPDFReweight() << endl;
    FillHist("NewPDF_PDFReweight", GetPDFReweight(), 1., 2000, 0.90, 1.10);
    //cout << "[TestIncludeAnaInAna::executeEvent] PDF reweight for error set (NErrorSet = "<<pdfReweight->NErrorSet<< ") :" << endl;
    for(int i=0; i<pdfReweight->NErrorSet; i++){
      //cout << "[TestIncludeAnaInAna::executeEvent]   " << GetPDFReweight(i) << endl;
...
void TestIncludeAnaInAna::executeEventFromParameter(AnalyzerParameter param){
  ...
  if(param.syst_ == AnalyzerParameter::Central){
    ...
    cout << "[TestIncludeAnaInAna::executeEventFromParameter] Wrong syst" << endl;
    exit(EXIT_FAILURE);
  }
