...
void DNNOptimizer::initializeAnalyzer(){
  cout << "[DNNOptimizer::initializeAnalyzer]" << endl;
  ...
  */
  ...
  if(GetEra()=="2016preVFP"){
    cut_jetcharge_dot_dnn=0.099;
  }
  else if (GetEra()=="2016postVFP"){
    cut_jetcharge_dot_dnn=0.091;
  }
  else if (GetEra()=="2017"){
    cut_jetcharge_dot_dnn=0.097;
  }
  else if (GetEra()=="2018"){
    cut_jetcharge_dot_dnn=0.093;
  }
...
void DNNOptimizer::SetBranches(TTree *this_Tree){
...
void DNNOptimizer::SetMuon(const Muon& _l1, const Muon& _l2){
  mu1=_l1;
  mu2=_l2;
  
}

bool DNNOptimizer::CheckIsDiMuonChannel(double min_mll,double max_mll){
...
void DNNOptimizer::SetElectron(const Electron& _l1, const Electron& _l2){
  el1=_l1;
  el2=_l2;
...
bool DNNOptimizer::CheckIsDiElectronChannel(double min_mll,double max_mll){
...
void DNNOptimizer::SetEventWeight(){
  weight=1;
  if(IsDATA) return;
...
void DNNOptimizer::RunBasicZregion(){
...
void DNNOptimizer::FillHistAll_bmuon(TString cutname,bmuonvar this_bmuon){
  ...
  FillHist(cutname+"/bmuon_chargescore_dot_charge_fine",this_bmuon.charge*GetMuonChargeScore(),weight,2400,-1.2,1.2);
  FillHist(cutname+"/bmuon_chargescore_dot_charge",this_bmuon.charge*GetMuonChargeScore(),weight,120,-1.2,1.2);
  FillHist(cutname+"/bmuon_chargescore_dot_int_bjetcharge",v_bjet[0].Charge() > 0 ? GetMuonChargeScore() : -GetMuonChargeScore() ,weight,120,-1.2,1.2);
  FillHist(cutname+"/bmuon_chargescore_dot_int_bjetcharge_fine",v_bjet[0].Charge() > 0 ? GetMuonChargeScore() : -GetMuonChargeScore() ,weight,2400,-1.2,1.2);
  FillHist(cutname+"/bmuon_chargescore_coeff",GetMuonChargeScoreCoeff(),weight,5,-3,2);
  if(fabs(GetMuonChargeScoreCoeff()) > 0 ) {//--- good muon
    FillHist(cutname+"/bmuon_measured_charge",this_bmuon.charge*GetMuonChargeScoreCoeff(),weight,5,-3,2);
  }
  if(GetMuonChargeScoreCoeff()>0){ //---good muon but excluding very low score with oppo sign
    FillHist(cutname+"/bmuon_measured_charge_no_opppsite",this_bmuon.charge*GetMuonChargeScoreCoeff(),weight,5,-3,2);
    FillHist(cutname+"/bmuon_measured_bjetcharge",v_bjet[0].Charge() > 0 ? 1 : -1,weight,5,-3,2);
  }
}

void DNNOptimizer::FillHistAll_belectron(TString cutname,belectronvar this_belectron){
  ...
  FillHist(cutname+"/belectron_chargescore_dot_charge_fine",this_belectron.charge*GetElectronChargeScore(),weight,2400,-1.2,1.2);
  FillHist(cutname+"/belectron_chargescore_dot_charge",this_belectron.charge*GetElectronChargeScore(),weight,120,-1.2,1.2);
  FillHist(cutname+"/belectron_chargescore_dot_int_bjetcharge",v_bjet[0].Charge() > 0 ? GetElectronChargeScore() : -GetElectronChargeScore() ,weight,120,-1.2,1.2);
  FillHist(cutname+"/belectron_chargescore_dot_int_bjetcharge_fine",v_bjet[0].Charge() > 0 ? GetElectronChargeScore() : -GetElectronChargeScore() ,weight,2400,-1.2,1.2);
  if(fabs(GetElectronChargeScoreCoeff()) > 0 ){
    FillHist(cutname+"/belectron_measured_charge",this_belectron.charge*GetElectronChargeScoreCoeff(),weight,5,-3,2);
  }
  if(GetElectronChargeScoreCoeff() > 0 ) {
    FillHist(cutname+"/belectron_measured_charge_no_opposite",this_belectron.charge*GetElectronChargeScoreCoeff(),weight,5,-3,2);
    FillHist(cutname+"/belectron_measured_bjetcharge",v_bjet[0].Charge() > 0 ? 1 : -1,weight,5,-3,2);
  }
}

void DNNOptimizer::FillHistAll_bjet(TString cutname,bjetvar this_bjet){
  ...
  FillHist(cutname+"/bjet_chargescore_dot_charge_fine",this_bjet.charge*GetJetChargeScore(),weight,2400,-1.2,1.2);
  FillHist(cutname+"/bjet_chargescore_dot_int_charge_fine",this_bjet.charge>0 ? GetJetChargeScore() : -GetJetChargeScore(),weight,2400,-1.2,1.2);
  FillHist(cutname+"/bjet_chargescore_dot_int_charge",this_bjet.charge>0 ? GetJetChargeScore() : -GetJetChargeScore(),weight,120,-1.2,1.2);
  if(fabs(GetJetChargeScoreCoeff()) > 0 ) {
    FillHist(cutname+"/bjet_measured_charge_HighScore",this_bjet.charge*GetJetChargeScoreCoeff(),weight,120,-1.2,1.2);
    FillHist(cutname+"/bjet_measured_int_charge_HighScore",this_bjet.charge*GetJetChargeScoreCoeff() > 0 ? 1 : -1,weight,4,-2,2);
  }
  else{
    FillHist(cutname+"/bjet_measured_charge_LowScore",this_bjet.charge,weight,120,-1.2,1.2);
    FillHist(cutname+"/bjet_measured_int_charge_LowScore",this_bjet.charge > 0 ? 1 : -1 ,weight,4,-2,2);
    FillHist(cutname+"/bjet_chargescore_dot_int_charge_fine_LowScore",this_bjet.charge>0 ? GetJetChargeScore() : -GetJetChargeScore(),weight,2400,-1.2,1.2);
  }
  if(GetJetChargeScore()*fabs(this_bjet.charge) > cut_jetcharge_dot_dnn){
    FillHist(cutname+"/bjet_int_charge_pass_jetcharge_dot_dnn",this_bjet.charge>0 ? 1 : -1 ,weight,4,-2,2);
  }
  else{
    FillHist(cutname+"/bjet_int_charge_fail_jetcharge_dot_dnn",this_bjet.charge>0 ? 1 : -1 ,weight,4,-2,2);
  }

}

void DNNOptimizer::FillHistAllChannel(TString cutname){
  if(check_tmva_input) return;
  DNNOptimizer::FillHistAll("ll__"+cutname);
  DNNOptimizer::FillHistAll(LepCh+"__"+cutname);
}


void DNNOptimizer::FillHistAll(TString cutname){
...
void DNNOptimizer::FillHist(TString histname, double value, double weight, int n_bin, double x_min, double x_max){
