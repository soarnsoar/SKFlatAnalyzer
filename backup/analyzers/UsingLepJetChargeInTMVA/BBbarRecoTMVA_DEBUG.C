#include "BBbarRecoTMVA_DEBUG.h"
#include <thread>
#include <chrono>


BBbarRecoTMVA_DEBUG::BBbarRecoTMVA_DEBUG(){//FYI : bottomness = -nb
...
void BBbarRecoTMVA_DEBUG::initTMVAmodel_odd_pos(){
  ...
  myreader_odd_pos->EvaluateMVA("PyKeras::DNN");
  //std::this_thread::sleep_for(std::chrono::milliseconds(30*1000));
  cout << "[done]bookmva"<< endl;
...
void BBbarRecoTMVA_DEBUG::initTMVAmodel_even_pos(){
  ...
  //std::this_thread::sleep_for(std::chrono::milliseconds(30*1000));
  myreader_even_pos->EvaluateMVA("PyKeras::DNN");
  cout << "[done]bookmva"<< endl;
  //Float_t ret=myreader->EvaluateMVA("PyKeras::DNN");
...
void BBbarRecoTMVA_DEBUG::initTMVAmodel_even_neg(){
  ...
  //std::this_thread::sleep_for(std::chrono::milliseconds(30*1000));
  myreader_even_neg->EvaluateMVA("PyKeras::DNN");
  cout << "[done]bookmva"<< endl;
  //Float_t ret=myreader->EvaluateMVA("PyKeras::DNN");
...
void BBbarRecoTMVA_DEBUG::initTMVAmodel_odd_neg(){
  ...
  myreader_odd_neg->EvaluateMVA("PyKeras::DNN");
  //std::this_thread::sleep_for(std::chrono::milliseconds(30*1000));
  cout << "[done]bookmva"<< endl;
  //Float_t ret=myreader->EvaluateMVA("PyKeras::DNN");
...
void BBbarRecoTMVA_DEBUG::InitTreeValues(){
...
void BBbarRecoTMVA_DEBUG::print(TString a){
  cout << a << endl;
}

void BBbarRecoTMVA_DEBUG::initializeAnalyzer(){
  i_EVENT=0;
  //==== if you use "--userflags RunSyst" with SKFlat.py, HasFlag("RunSyst") will return "true"
  RunSyst = HasFlag("RunSyst");
  cout << "[BBbarRecoTMVA_DEBUG::initializeAnalyzer] RunSyst = " << RunSyst << endl;
  ...
  cout << "[BBbarRecoTMVA_DEBUG::initializeAnalyzer Setting ProcessName = " << ProcessName << endl;
...
BBbarRecoTMVA_DEBUG::~BBbarRecoTMVA_DEBUG(){

  //==== Destructor of this Analyzer

}

bool BBbarRecoTMVA_DEBUG::Tag_gbToZb(){
  print("<Tag_gbToZb>");
  LHEs=GetLHEs();
  BBbarRecoTMVA_DEBUG::myLHE.LHEsize = LHEs.size();  
...
int BBbarRecoTMVA_DEBUG::Rank1n2Leptons(std::vector<int> &v_blep_idx, std::vector<double> &v_blep_value){
  double center=1.7;
...
void BBbarRecoTMVA_DEBUG::FillHistMuon(TString cutname){
...
void BBbarRecoTMVA_DEBUG::FillHistMuonCharge(TString cutname){
...
void BBbarRecoTMVA_DEBUG::RunLeptonCutStudyMuon(){
...
void BBbarRecoTMVA_DEBUG::SetTreeValuesMuon(int i_bmuon1,int i_bmuon2,int nbmuon){
...
void BBbarRecoTMVA_DEBUG::FillHistElectron(TString cutname){
...
void BBbarRecoTMVA_DEBUG::FillHistElectronCharge(TString cutname){
...
void BBbarRecoTMVA_DEBUG::RunLeptonCutStudyElectron(){
  print("in RunLeptonCutStudyElectron");
  ...
  if(v_tmva_belectronidx.size()>0){
    ...
    if(v_tmva_belectronidx.size()==1){
      print("if 1tmva electron");
      SetTreeValuesElectron(i_belectron,-1,1);
    }//[END] #electron==1
    else{
      print("more than 2tmva electron");
      print("rank lepton 1, 2");
  ...
  print("end of if tmva belectron");

}



void BBbarRecoTMVA_DEBUG::SetTreeValuesElectron(int i_belectron1,int i_belectron2,int nbelectron){
  print("SetTreeValuesElectron");
...
bool BBbarRecoTMVA_DEBUG::Tag1bjet(){
...
void BBbarRecoTMVA_DEBUG::AnalyzeRECO(){
  ...
  FillHist("PassTrigger",1, weight, 1, 0, 2);
  if(myRECO.passMuonTrigger){
    FillHist("PassMuonTrigger",1, weight, 1, 0, 2);
    if (!ZmmReco()) return;
    FillHist("PassMuonTrigger__and__GoodZmm",1, weight, 1, 0, 2);
  }

  else if(myRECO.passElectronTrigger){
    FillHist("PassElectronTrigger",1, weight, 1, 0, 2);
    if (!ZeeReco()) return;
    FillHist("PassElectronTrigger__and__GoodZee",1, weight, 1, 0, 2);
    print("pass electron trigger and good Zee");
  }
  FillHist("AfterGoodZ",1, weight, 1, 0, 2);
  print("Tag1bjet");
  if(!BBbarRecoTMVA_DEBUG::Tag1bjet()) return;
  FillHist("Pass1bjet",1, weight, 1, 0, 2);
  //->now Z is reconstructed
  //(1) Do Main Analysis
  //(1-1) Set EventTag
  //(1-2)For B hadron to lepton decay
  //BBbarRecoTMVA_DEBUG::RunProtoTypeMuon();
  //BBbarRecoTMVA_DEBUG::RunProtoTypeElectron();
  print("RunLeptonCutStudyMuon");
  BBbarRecoTMVA_DEBUG::RunLeptonCutStudyMuon();
  FillHist("PassRunLeptonCutStudyMuon",1, weight, 1, 0, 2);
  print("RunLeptonCutStudyElectron");
  BBbarRecoTMVA_DEBUG::RunLeptonCutStudyElectron();
  FillHist("PassRunLeptonCutStudyElectron",1, weight, 1, 0, 2);

}

double BBbarRecoTMVA_DEBUG::CalcDNN(){
...
void BBbarRecoTMVA_DEBUG::executeEvent(){
  i_EVENT+=1;
  if (i_EVENT!=209844) return;
  cout << i_EVENT << endl;
  ...
  if ("DY"==ProcessName){
    myLHE.is_gbToZb = BBbarRecoTMVA_DEBUG::Tag_gbToZb(); 
    //BBbarRecoTMVA_DEBUG::AnalyzeLHE();
  }
  print("before AnalyzeRECO");
  BBbarRecoTMVA_DEBUG::AnalyzeRECO();
  //NowFillTree
  if(doFillTree){
    print("doFillTree");
    ...
    if(myRECO.passMuonTrigger && myRECO.goodZmm){
      print("pass muon trigger fillhist");
    ...
    if(myRECO.passElectronTrigger && myRECO.goodZee){
      print("pass electron trigger fillhist");
      ...
      print("[end]pass electron trigger fillhist");
    }
    
    //---
    if(bmuon1_pt>0){
      print("bmuon1_pt  fillhist");
      ...
      print("[end]bmuon1_pt  fillhist");
    }
    if(bmuon2_pt>0){
      print("bmuon2_pt  fillhist");
      ...
      print("[end]bmuon2_pt  fillhist");
    }
    if(belectron1_pt>0){
      print("belectron1_pt  fillhist");
      ...
      print("[end]belectron1_pt  fillhist");
    }

    if(belectron2_pt>0){
      print("belectron2_pt  fillhist");
      ...
      print("[end]belectron2_pt  fillhist");
    }
    print("fillhist bjet");
    ...
    print("[end]fillhist bjet");
  }//end dofilltree
  print("[end]dofilltree");
  //  FillHist("BasicCut/ZCand_Mass/"+ProcessName, ZCand.M(), weight, 40, 70., 110.);
  
    print("[end]execute event");
}
